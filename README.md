...
- [pxt-microbit-ml](https://github.com/microbit-foundation/pxt-microbit-ml) —
  the MakeCode extension this app adds to the projects it generates (the
  version is pinned in [src/makecode/utils.ts](src/makecode/utils.ts)). It
  provides the ML blocks and on-device inference runtime, plus the simulator
  extension shown alongside the micro:bit simulator in MakeCode. The
  `ml.event.*` blocks for the user's trained actions don't live in the
  extension itself: this app generates them into each project's
  `autogenerated.ts` from the trained model.
- [pxt-microbit-ml-help-stubs](https://github.com/microbit-foundation/pxt-microbit-ml-help-stubs)
  — a stub extension used only by pxt-microbit-ml's MakeCode help pages. Since
  the `ml.event.*` action constants are generated per-project (see above), the
  documentation's example snippets wouldn't compile against the bare
  extension; the stubs supply an example action so the snippets render as
  blocks in the docs.
