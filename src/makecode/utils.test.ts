...
describe("test project update", () => {
  ...
  it("lists the generated files in pxt.json for a project created in MakeCode", () => {
    // A hex from MakeCode has no ML extension and none of our files.
    const makeCodeFiles = ["main.blocks", "main.ts", "README.md", "pxt.json"];
    const makeCodeProject: MakeCodeProject = {
      text: {
        ...
        [filenames.mainTs]: 'basic.showString("Hello!")\n',
        [filenames.mainBlocks]: "<xml></xml>",
        [filenames.readme]: "",
      },
    };

    const customFiles = generateCustomFiles(
      data,
      trainedModel,
      currentDataWindow,
      true,
    ...
    const updatedPxt = JSON.parse(customFiles[filenames.pxtJson]) as typeof pxt;

    // MakeCode only compiles files listed here, so the ML event
    // definitions in autogenerated.ts must be listed to appear in blocks.
    expect(updatedPxt.files).toEqual([
      ...makeCodeFiles,
      filenames.autogenerated,
      filenames.datasetJson,
      filenames.metadata,
    ]);
    expect(updatedPxt.dependencies[extensionName]).toEqual(
      pxt.dependencies[extensionName]
    );
    expect(customFiles[filenames.autogenerated]).toContain("fixedInstance");
  });
});
