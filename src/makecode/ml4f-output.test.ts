...
test("spits out files useful for testing", async () => {
  ...
  // Train on the training data; testing data is derived from the same features.
  const { trainingResult, features, labels } = await trainModelForTest(
    trainingData,
    dataWindow
  );
