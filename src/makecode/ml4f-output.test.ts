...
import rawTrainingData from "../test-fixtures/microbit-AI-activity-timer-data-log-mth-1s.json";
...
test("spits out files useful for testing", async () => {
  ...
  const { features, labels } = prepareFeaturesAndLabels(trainingData);
  const testingData = {
    x: features,
    y: labels,
  };
  ...
  const samples_period = 20;
  const samples_length = averageSamples;
