...
function convertTestDataToC(testData: GestureData[], filterOutput: number[][], ml4fOutput: number[][]) {
  const xData: number[][] = [];
  const yData: number[][] = [];
  const zData: number[][] = [];
...
test("spits out files useful for testing", async () => {
  ...
  writeFileSync("testData.h", convertTestDataToC(trainingData as GestureData[], features, ml4fOutput));
});
