...
import trainingData from "../test-fixtures/microbit-AI-activity-timer-normal-data-log.json";
...
test("spits out files useful for testing", async () => {
  ...
  const ml4fOutput = [];
  for (let i = 0; i < ml4fPredictionResultArry.length; i += 5) {
    if (i + 5 > ml4fPredictionResultArry.length) {
      break;
    }
